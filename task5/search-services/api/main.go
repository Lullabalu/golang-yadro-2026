...
func main() {
	...
	if err != nil {
		log.Error("cannot init update apater", "error", err)
		os.Exit(1)
	}
	...
	mux.HandleFunc("GET /api/ping", rest.NewPingHandler(log, map[string]core.Pinger{"update": updateClient, "words": wordsClient, "search": searchClient}))
